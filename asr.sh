...
word_vocab_size=20000 # Size of word vocabulary.
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        if [ "${feats_type}" = raw ]; then
            ...
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                ...
                copy_data_dir.sh --validate_opts --non-print data/"${dset}" "${data_feats}${_suf}/${dset}"
            ...
            if [ "$pretrain" = true ]; then
                sed -i "s% .*.ark% ${raw_feats_dir}/pretrain.ark%g" ${raw_feats_dir}/pretrain.scp 
            fi

            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                mkdir -p data/"${dset}"
                sed -i "s% .*.ark% ${raw_feats_dir}/${dset}.ark%g" ${raw_feats_dir}/${dset}.scp 
            done
            python lrs2_prepare.py ${raw_feats_dir} ${pretrain}
            ...
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do 
                ...
                # lrs2_local/fix_data_dir.sh "${data_feats}${_suf}/${dset}"
            ...
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                ...
                python pyscripts/feats/feat-to-shape.py "scp:head -n 1 ${data_feats}${_suf}/${dset}/feats.scp |" - | \
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        for dset in "${train_set}" "${valid_set}"; do
            ...
            # Copy data dir
            copy_data_dir.sh --validate_opts --non-print "${data_feats}/org/${dset}" "${data_feats}/${dset}"
            ...
            fix_data_dir.sh "${data_feats}/${dset}"
        done
